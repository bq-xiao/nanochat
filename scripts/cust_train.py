...
from nanochat.cust_dataset import get_dataloader
from nanochat.tokenization_chatglm import ChatGLMTokenizer
...
from nanochat.common import compute_init, compute_cleanup, print0, DummyWandb, autodetect_device_type, get_peak_flops
...
tokenizer = ChatGLMTokenizer.from_pretrained(args.tokenizer_model)
...
model_config_kwargs = dict(sequence_len=args.max_seq_len, vocab_size=vocab_size, n_layer=num_layers, n_head=num_heads,
                           n_kv_head=num_kv_heads, n_embd=model_dim, window_pattern=args.window_pattern)
...
num_scaling_params = param_counts['transformer_matrices'] + param_counts[
    'lm_head']  # determined to give the cleanest scaling laws, see dev/LOG.md Jan 27, 2026
...
print0(
    f"Tokens : Scaling params ratio: {args.total_batch_size * num_iterations / num_scaling_params:.2f}")  # Chinchilla is ~20
