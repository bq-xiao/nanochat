...
class SPTokenizer:
    def __init__(self, model_path: str):
        ...
        self.role_special_token_expression = "|".join(
            [re.escape(token) for token in special_tokens])  # for apply_chat_template
...
class ChatGLMTokenizer(PreTrainedTokenizer):
    ...
    def bos_token(self) -> str:
        return self.tokenizer.sp_model.IdToPiece(self.get_command("<bos>"))
    ...
    def get_token_bytes(self, device="cpu"):
        ...
        special_set = set(self.tokenizer.special_tokens.keys())
        token_strings = [self.tokenizer.decode([token_id]) for token_id in range(vocab_size)]
        token_bytes = []
        ...
        token_bytes = torch.tensor(token_bytes, dtype=torch.int32, device=device)
        return token_bytes
